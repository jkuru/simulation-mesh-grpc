// external-risk composition root — real third-party stand-in.
// Scoring rules live in internal/risk (TokenPrefixScorer).
package main
...
func main() {
	...
	port := env.Get("GRPC_PORT", "9003")

	svc := risk.NewExternalService(log, risk.TokenPrefixScorer{})
	...
	go func() {
		log.Info("external-risk listening", "port", port)
		errCh <- gs.Serve(lis)
	}()
